...
#-------------------------------------------------------------------------------
    class izhikevich():
        def __init__(self,A=0.04,B=5,C=140,Cm=1,a=0.02,b=0.2,c=-65,d=2,name='default'):
            ...
            self.spike_idx = 0 #initial value for spike index
        ...
        def integrate(self,input_current,dt):
            ...
            self.vm = v_old + (dt/2)*((self.A*v_old*v_old + self.B*v_old - u_old + self.C + (input_current/self.Cm)));
            self.vm = self.vm + (dt/2)*((self.A*v_old*v_old + self.B*v_old - u_old + self.C + (input_current/self.Cm)));
            #find the next value of the recovery variable
            self.u = u_old + dt*self.a*((self.b * v_old) - u_old);
            #spike event
            #if a spike is generated
            self.ineuron.append(input_current)
            ...
            if self.vm > 30:
                ...
                self.uv.append(self.u)
                # print(self.u, self.d)
                self.spikes.append(self.spike_idx)
                self.vneuron.append(self.vm)
                self.spike_idx += 1
...
class classification():
    ...
    def LOOCV(classifierObj,features,targets,target_names=None):
        ...
        if target_names is not None:
            # confm = confusion_matrix(y_true,y_pred,labels=target_names)
